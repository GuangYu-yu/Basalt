#!/bin/bash
# =============================================================================
# Basalt — 网络后端稳定性回归测试（slirp | passt）
# =============================================================================
# 目的：验证测试网络后端（LANDSCAPE_TEST_NET=passt|slirp）在"guest 长时间
# 存活 + 周期性 SSH 探测 + guest→宿主 HTTP 拉取"下的稳定性，防止回归到
# slirp 的网络路径死亡形态（guest 健康、sshd 在列、hostfwd 在列但新建连接
# 不通）。最小网络闭环（boot → bootstrap → SSH → guest 拉取宿主 HTTP →
# 驻留期周期探测）连跑 N 轮，任一网络死亡即 FAIL。
#
# 用法：test-net-stress.sh <image_path>   （env：ITERATIONS=6 DWELL=300）
# 判定：全部轮次 bootstrap/SSH/HTTP 探测通过 → PASS；任一网络死亡 → FAIL
#（失败轮次自动经 emergency/console 探针取证——与 OTA 测试共用取证链）
# =============================================================================
set -euo pipefail
...
# guest→宿主 HTTP 拉取探测（本地源的 SHA256SUMS 占位文件——验证 guest 经
# WAN 网关访问宿主服务的完整路径）
probe_http() {
...
main() {
    info "网络后端回归：backend=$(landscape_net_backend) iterations=${ITERATIONS} dwell=${DWELL}s"
    ...
    echo ""
    if (( fails == 0 )); then
        ok "网络后端回归全部通过（${ITERATIONS} 迭代，backend=$(landscape_net_backend)）"
    else
        error "网络后端回归失败（迭代 ${i}，backend=$(landscape_net_backend)）"
        exit 1
    fi
}
